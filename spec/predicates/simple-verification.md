...
A Simple Verification Attestation (SVA) communicates that an artifact has been verified against one or more policies, and records the properties that were verified. This enables consumers to trust a concise summary of verification results, without requiring access to all underlying attestations or policies.
...
```jsonc
{
  ...
  "predicate": {
    ...
    "properties": ["<SCOPED_VERIFIED_PROPERTY>", ...]
  }
}
```
...
**`timeVerified`, required** string (Timestamp)
...
> Indicates the properties verified for the artifact.
